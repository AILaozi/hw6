...
private:
    ...
    double resizeAlpha_;
    HASH_INDEX_T activeNums_;
    HASH_INDEX_T deletedNums_;
...
{
  double fraction = static_cast<double>(activeNums_ + deletedNums_) / CAPACITIES[mIndex_];
  if (fraction >= resizeAlpha_) resize();
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  for (HASH_INDEX_T i = 0; i < old.size(); i++) {
    HashItem* curr = old[i];

    if (curr != nullptr) {
      if (curr->deleted == false) {
        HASH_INDEX_T loc = probe(curr->item.first);
        table_[loc] = curr;
        activeNums_ += 1;
      }
      else {
